...
class OVStitcherSegmentation(BaseSegmentor):
    def __init__(self, clip_type, model_type, dino_type, name_path, device=torch.device('cuda'),
                 prob_thd=0.0, logit_scale=40, slide_stride=112, slide_crop=336, instance_mask_path=None, mask_generator=None, template_path=None):
        ...
        if isinstance(patch_size, tuple):
            if patch_size[0] != patch_size[1]:
                raise ValueError('DINO patch size must be square')
    ...
    def forward_feature(self, img, coords, paddings, masks):
        if type(img) == list:
            img = img[0]
        ...
        def capture_qkv(module, inputs, output):
            feat_out['qkv'] = output

        qkv_layer = self.dino.blocks[-1].attn.qkv
        hook = qkv_layer.register_forward_hook(capture_qkv)
        try:
            feat_penul = self.dino.get_intermediate_layers(imgs_norm, n=2)[0]
        finally:
            hook.remove()

        nb_im = feat_penul.shape[0]  # Batch size
        nb_tokens = feat_penul.shape[1]  # Number of tokens

        qkv = feat_out["qkv"][:, 1:].reshape(nb_im, nb_tokens-1, 3, -1).permute(2, 0, 1, 3)

        dino_q = qkv[0]
        dino_k = qkv[1]
        dino_v = qkv[2]

        stitched_dino_q = self.stitch_features_from_batch(dino_q, coords, paddings, encoder='dino')[0]
        stitched_dino_k = self.stitch_features_from_batch(dino_k, coords, paddings, encoder='dino')[0]
        stitched_dino_v = self.stitch_features_from_batch(dino_v, coords, paddings, encoder='dino')[0]

        dino_head_dim = self.dino.embed_dim // self.dino.blocks[-1].attn.num_heads
        dino_scale = dino_head_dim ** -0.5

        stitched_dino_q = stitched_dino_q.contiguous().view(-1, self.dino.blocks[-1].attn.num_heads, dino_head_dim).transpose(0, 1).to(stitched_dino_v.dtype)
        stitched_dino_k = stitched_dino_k.contiguous().view(-1, self.dino.blocks[-1].attn.num_heads, dino_head_dim).transpose(0, 1).to(stitched_dino_v.dtype)
        stitched_dino_v = stitched_dino_v.contiguous().view(-1, self.dino.blocks[-1].attn.num_heads, dino_head_dim).transpose(0, 1)

        attn = torch.bmm(stitched_dino_q, stitched_dino_k.transpose(1, 2)) * dino_scale
        attn = F.softmax(attn, dim=-1)
        attn = torch.bmm(attn, stitched_dino_v)
        attn = attn.transpose(0, 1).contiguous().view(-1, 1, self.dino.embed_dim)

        feat_penul, feat_shape = self.stitch_features_from_batch(feat_penul[:, 1:], coords, paddings, encoder='dino')

        dino_feats = feat_penul + F.linear(attn.permute(1, 0, 2), self.dino.blocks[-1].attn.proj.weight, self.dino.blocks[-1].attn.proj.bias)

        dino_feats = dino_feats + F.linear(
            F.gelu(F.linear(
                F.layer_norm(
                    dino_feats,
                    normalized_shape=self.dino.blocks[-1].norm2.normalized_shape,
                    weight=self.dino.blocks[-1].norm2.weight,
                    bias=self.dino.blocks[-1].norm2.bias,
                    eps=self.dino.blocks[-1].norm2.eps,
                ),
                weight=self.dino.blocks[-1].mlp.fc1.weight,
                bias=self.dino.blocks[-1].mlp.fc1.bias,
            )),
            weight=self.dino.blocks[-1].mlp.fc2.weight,
            bias=self.dino.blocks[-1].mlp.fc2.bias,
        )

        dino_feats = F.normalize(dino_feats, dim=-1)   # N_crop, N_patch, D

        image_features = self.clip.encode_image(self.img_shape, img.half(), dino_feats=dino_feats, dino_patch_size = self.dino_patch_size, feat_shape=feat_shape, 
                                                instance_masks=masks, coords=coords, paddings=paddings, slide_stride = self.slide_stride, slide_crop = self.slide_crop)
        ...
        logits = nn.functional.interpolate(logits, size=self.img_shape[-2:], mode='bilinear')
    ...
    def get_windowed_imgs(self, img):
        ...
        stride = ((self.slide_stride, self.slide_stride)
                  if isinstance(self.slide_stride, int) else self.slide_stride)
        crop_size = ((self.slide_crop, self.slide_crop)
                     if isinstance(self.slide_crop, int) else self.slide_crop)
    ...
    def stitch_features_from_batch(self, feature, pixel_locs, paddings, encoder='dino'):
        if encoder == 'dino':
            ...
            if isinstance(patch_size, tuple):
                if patch_size[0] != patch_size[1]:
                    raise ValueError('CLIP patch size must be square')
                patch_size = patch_size[0]
        else:
        ...
        # permute to [num_crop, D, num_token]
        feature = feature.permute(0, 2, 1)  # [num_crop, D, num_token]

        H_feat = self.img_shape[2] // patch_size
        W_feat = self.img_shape[3] // patch_size
        ...
        for i, (loc, pad) in enumerate(zip(pixel_locs, paddings)):
            ...
            valid_h = crop_feat_h - (pad_t_patch + pad_b_patch)
            valid_w = crop_feat_w - (pad_l_patch + pad_r_patch)

            feat = feature[i].reshape(D, crop_feat_h, crop_feat_w)

            feat = feat[:, pad_t_patch:pad_t_patch+valid_h, pad_l_patch:pad_l_patch+valid_w] # 34
        ...
        return stitched_feature.reshape(1, D, -1).permute(0, 2, 1), stitched_feature.shape[-2:]

    def forward_slide(self, img, instance_masks, img_metas):
        if type(img) == list:
            img = img[0]

        cropped_images, coords, paddings = self.get_windowed_imgs(img)
        preds = self.forward_feature(cropped_images, coords, paddings, instance_masks) # B, Class, H, W
    ...
    def predict(self, inputs, data_samples):
        ...
        seg_logits = self.forward_slide(inputs, instance_masks, batch_img_metas)
    ...
    def generate_category_embeddings(self, name_path, device=torch.device('cuda')):
        if self.template_path is not None:
            print('CBT with', self.template_path)
            class_biased_templates = load_json(self.template_path)
        ...
        with torch.inference_mode():
            for qw in query_words:
                if self.template_path is not None:
                    if qw in class_biased_templates.keys():
                        additional_templates = class_biased_templates[qw]
                    else:
                        print(qw, "not in class_biased_templates")
                        additional_templates = []
                if self.template_path is not None and len(additional_templates) > 0:
                    query = self.tokenizer([temp(qw) for temp in openai_imagenet_template]+additional_templates).to(device)
                else:
...
def measure_predict_memory(model, inputs, data_samples=None):
    ...
    return (peak - start_mem) / 1024**2  # MB
