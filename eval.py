...
def parse_args():
    parser = argparse.ArgumentParser(
        description='OV-Stitcher evaluation with MMSeg')
    parser.add_argument('--config', default='./configs/cfg_voc21.py')
    parser.add_argument(
        '--show', action='store_true', help='show prediction results')
    parser.add_argument(
        '--show-dir', '--show_dir', dest='show_dir',
        default=None,
        help='directory to save visualization images (disabled by default)')
    parser.add_argument(
        '--wait-time', type=float, default=0,
        help='display time in seconds when using --show')
    parser.add_argument(
        '--out',
        type=str,
...
def trigger_visualization_hook(cfg, args):
    if args.show or args.show_dir:
        cfg.default_hooks['visualization'] = dict(
            type='SegVisualizationHook', draw=True, interval=5,
            show=args.show, wait_time=args.wait_time)
        if args.show_dir:
